...
import { ExportApprovalDataType } from '../Interface/IApprovalEngagement';
    ...
    if (updatedData.length > 0 && masterData.length > 0) {
      const mappedData = updatedData.map((item) => {
        ...
        return{
          ...
          updatedDate: String(formatDate(item.dateUpdated, 'dd LLLL yyyy, HH:mm')),
...
export function ApprovalEngagementHistoryTable() {
  const userProfile = useSelector(selectProfile);
  const { currentRoleDetailId, rolePermissions, binusianId } = userProfile;
  ...
  const getExportApproval = useCallback(
    async () => {
      ...
      apiClient.get(`${ApiService.engagement}/export-approval?userRoleDetail=${currentRoleDetailId}&startPeriod=${tempStartDate ?? ''}&endPeriod=${tempEndDate ?? ''}`)
      .then(resp=>resp)
      .then(resp=>{
  ...
  const HandleExportToExcel = () => {
    setLoadingExport(true);
    const headers = ['Period', 'Date', 'NIM', 'Name', 'Campus', 'Faculty', 'Program', 'Degree', 'Unit Name', 'Category', 'Category Detail', 'Nominal', 'Activity', 'Link Evidence', 'Description', 'Status', 'Action Type', 'Reason', 'Send Back Reason', 'Requested Date', 'Requested By'];
  ...
  return (
    <PageWrapper>
      ...
      <Stack direction='row' gap='10px' sx={{width: '100%'}}>
        {rolePermissions.some(item => item.permissionName === 'export-approval-engagement') && (
          <CustomLoadingButton
...
ApprovalEngagementHistoryTable.propTypes = null;
