  ...
  uniqueUnit
    ...
    useEffect(() => {
        if(textFieldRef.current){
            textFieldRef.current.value = (rowName == 'debit' || rowName == 'kredit') ? 
    ...
    const textFieldChangeHandler = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (name == 'debit' && textFieldRef.current) {
            textFieldRef.current.value = formatNumber(value);
        }
        else if(name == 'kredit' && textFieldRef.current){
            textFieldRef.current.value = formatNumber(value);
        }
    ...
    const fetchAlumniDetails = async (nimValue: string) => {
        setTimeout(async () => {
            try {
                ...
                const response = await apiClient.post(`${ApiService.endowment}/alumni-detail`, tempNIM);
    ...
    const touched = formik.touched.items?.[index]?.[rowName];
    return(
        ['category', 'unitName'].includes(rowName) ? (
            <Select
            ...
            >
                {(rowName === 'category' ? uniqueEndowmentCategory : uniqueUnit).map((option) => (
    ...
    TableRowComponent.propTypes = {
        ...
        formik: undefined,
        uniqueEndowmentCategory: undefined,
