...
import { IUpdatedData } from './interface/Interface';
...
export function DetailDashboardByUpdatedData(){
	...
	const [updatedDataChart, setUpdatedDataChart] = useState<IUpdatedData[]>([]);
  ...
  const getProgramsByRole = useCallback(
	...
	const getUpdatedDataChart = async (useParam: boolean = false) => {
		const param = {
			CampusId: formik.values.filter.campusId,
			FacultyId: formik.values.filter.facultyId,
			ProgramId: formik.values.filter.programId,
		...
		const url = ApiService.dashboardUpdatedData + (useParam ? `?${qs.stringify(param)}` : '');
		const response = await apiClient.get(url);
		setUpdatedDataChart(response.data);
		setChartLoaded(true);
	};
		...
		return (
			<Stack sx={{ width: { xs: '90%', md: '80%' }, height: '100%' }}>
				<Typography
					...
					{ chartLoaded ? 'Chart Unavailable: No Alumni Data' : 'Loading Data...'}
				</Typography>
			</Stack>
		)
		}, [updatedDataChart]);
